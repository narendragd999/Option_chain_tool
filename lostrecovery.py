...
def check_touch_yesterday_open(suggestions: List[Dict], bot_token: str, chat_id: str, expiry: str) -> List[Dict]:
    triggered_notifications = []
    
    for suggestion in suggestions:
        ticker = suggestion["Ticker"]
        yesterday_open = suggestion["Yesterday_Open"]
        underlying = suggestion["Underlying"]
        lost_momentum_detected = suggestion["Lost_Momentum_Detected"]
        
        # Check if price has touched or crossed yesterday_open after lost momentum
        if lost_momentum_detected and not suggestion["Touched_Yesterday_Open"]:
            # Define "touch" as within a small range (e.g., ±0.5% of yesterday_open)
            touch_threshold = yesterday_open * 0.005  # 0.5% threshold, adjust as needed
            if abs(underlying - yesterday_open) <= touch_threshold or \
               (underlying >= yesterday_open and suggestion["Underlying"] < yesterday_open):
                suggestion["Touched_Yesterday_Open"] = True
                message = (
                    f"*RECOVERY ALERT*\n"
                    f"Stock: *{ticker}*\n"
                    f"Yesterday Open: *₹{yesterday_open:.2f}*\n"
                    f"Current: *₹{underlying:.2f}*\n"
                    f"Reason: *Price touched or recovered to Yesterday Open after Lost Momentum*"
                )
                triggered_notifications.append({"Ticker": ticker, "Message": message})
    
    # Send Telegram notifications
    if triggered_notifications and bot_token and chat_id:
        for notification in triggered_notifications:
            asyncio.run(send_telegram_message(bot_token, chat_id, notification["Message"]))
...
def generate_lost_momentum_suggestions(tickers: List[str], expiry: str, bot_token: str, chat_id: str) -> List[Dict]:
    ...
    for ticker in tickers:
        with st.spinner(f"Fetching data for {ticker}..."):
            ...
            lost_momentum = yesterday_open > underlying
            if lost_momentum:
                otm_calls = call_df[call_df['Strike'] > underlying]
                if otm_calls.empty:
                ...
                suggestion = {
                    ...
                    "Suggestion": "Sell Call",
                    "Lost_Momentum_Detected": True,  # Flag for lost momentum
                    "Touched_Yesterday_Open": False  # Flag for later check
                }
                suggestions.append(suggestion)
            else:
                # If no lost momentum but price is close to yesterday_open, track it
                suggestion = {
                    "Ticker": ticker,
                    "Yesterday_Open": yesterday_open,
                    "Underlying": underlying,
                    "Strike": None,
                    "Premium": None,
                    "Reason": "Monitoring for touch after lost momentum",
                    "Suggestion": None,
                    "Lost_Momentum_Detected": False,
                    "Touched_Yesterday_Open": False
                }
                suggestions.append(suggestion)
...
def check_and_trigger_auto_scan():
    ...
    if (st.session_state['telegram_config']['auto_scan_enabled'] and 
        ...
        if momentum_tickers:
            ...
            # Check for touch after lost momentum
            if st.session_state.get('enable_telegram_alerts', False):
                momentum_suggestions = check_touch_yesterday_open(
                    momentum_suggestions,
                    st.session_state['telegram_config']['telegram_bot_token'],
                    st.session_state['telegram_config']['telegram_chat_id'],
                    st.session_state.get('expiry', '')
                )
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("Reset Momentum Suggestions"):
            st.session_state['momentum_suggestions'] = None
            st.rerun()
    ...
    with tabs[9]:
        ...
        # Define current_time here to ensure it’s available
        current_time = time.time()
        auto_scan_interval_seconds = st.session_state['telegram_config']['auto_scan_interval'] * 60
            ...
            if momentum_tickers:
                with st.spinner("Scanning for lost momentum..."):
                    ...
                    if enable_telegram_alerts:
                        suggestions = check_touch_yesterday_open(
                            suggestions, telegram_bot_token, telegram_chat_id, expiry
                        )
                    st.session_state['momentum_suggestions'] = suggestions
                    
                    # Send initial Lost Momentum Telegram message (optional, keep if desired)
                    if suggestions and st.session_state['telegram_config']['enable_telegram_alerts'] and telegram_bot_token and telegram_chat_id:
                        message = f"*Lost Momentum Suggestions (Auto-Scan every {st.session_state['telegram_config']['auto_scan_interval']} min)*\n"
                        for suggestion in suggestions:
                            if suggestion["Lost_Momentum_Detected"]:
                                message += (
                                    f"Stock: *{suggestion['Ticker']}*\n"
        ...
        if st.session_state['momentum_suggestions'] is not None:
            if st.session_state['momentum_suggestions']:
                ...
                suggestions_df["Status"] = suggestions_df.apply(
                    lambda row: "Touched Yesterday Open" if row["Touched_Yesterday_Open"] else 
                               "Lost Momentum" if row["Lost_Momentum_Detected"] else "Monitoring", 
                    axis=1
                )
                st.write("### Lost Momentum Trade Suggestions")
                styled_df = suggestions_df[["Ticker", "Yesterday_Open", "Underlying", "Strike", "Premium", "Reason", "Status"]].style.format({
                    'Yesterday_Open': '{:.2f}',
                    'Underlying': '{:.2f}',
                    'Strike': '{:.2f}',
                    'Premium': '{:.2f}'
